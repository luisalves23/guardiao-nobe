...
test('E2E UI & Graphical Interface: Bateria Completa com Navegador Real', async () => {
  ...
  try {
    ...
    // Validação da versão v2.7.1 no cabeçalho
    const versionBadge = await page.locator('text=v2.7.1').first();
    assert.ok(await versionBadge.isVisible(), 'Badge de versão v2.7.1 deve estar visível no topo');
    console.log('[E2E UI] ✅ Badge de versão v2.7.1 identificado com sucesso no cabeçalho');
