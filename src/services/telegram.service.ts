...
export class TelegramService {
  ...
  public async initialize(): Promise<void> {
    ...
    if (!token) {
      console.log('[Telegram] Bot token não configurado ainda. Aguardando configuração.');
      return;
    }
    ...
    try {
      ...
      if (res.data?.ok && Array.isArray(res.data.result)) {
        for (const update of res.data.result) {
          ...
          // Salva automaticamente o chatId se não estiver configurado
      ...
      // Ignora timeouts normais de long polling
    }
  ...
  public async handleIncomingMessage(chatId: string, text: string): Promise<string | void> {
    ...
    console.log(`[Telegram] Mensagem recebida do Chat ${chatId}: "${cleanText}"`);

    // 1. Se for comando com prefixo '/' ou '!'
    ...
    // Resposta padrão
    const defaultReply =
      '🤖 Olá Luís! Sou o Guardião Nobe. Use /status para ver o expediente ou aguarde minha checagem periódica.';
  ...
  public async askActivityQuestion(
    ...
    return new Promise<string | null>((resolve) => {
      ...
      this.sendAlert(questionMessage).catch((err) =>
        console.error('[Telegram] Falha ao enviar alerta de pergunta:', err)
      );
    });
  }
